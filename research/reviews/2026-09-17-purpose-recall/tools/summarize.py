...
spec = importlib.util.spec_from_file_location('construction_recall_evidence', PARENT/'tools/summarize.py')
...
def data_sets():
    ...
    sets['exposed_construction'] = sets.pop('confirmation')
...
def inherited_claims(split, profile):
    old_split = 'confirmation' if split == 'exposed_construction' else split
