"""Freeze confirmation pages by identity, excluding both earlier review sets."""
import argparse
import gzip
import hashlib
import io
...
def main():
    ...
    for cohort in ('ptah', 'historical'):
        for fmt in ('any',):
            for length in ('short', 'medium', 'long'):
                ...
                def rank(p):
                    return digest(('issue300-confirmation-v1\n'+p['reference']).encode())
    ...
    result = dict(version=1, selection='issue300-confirmation-v1', development_manifest_sha256=digest(development.read_bytes()),
