#!/usr/bin/env python3
"""Validate discourse-pattern inputs, judgments and full-page measurements."""
import argparse
...
PARENT = ROOT.parent/'2026-09-18-clause-grammar'
spec = importlib.util.spec_from_file_location('clause_grammar_evidence', PARENT/'tools/summarize.py')
...
def data_sets():
    ...
    sets['exposed_grammar'] = sets.pop('confirmation')
...
def inherited_claims(split, profile):
    old_split = 'confirmation' if split == 'exposed_grammar' else split
    ...
    for split, (pages, files, events) in sets.items():
        ...
        for profile in ('technical', 'strict'):
            ...
            if split == 'confirmation':
                summary['sensitivity'] = dict(method='No interval: only one long page per cohort; report observed counts only',
