#!/usr/bin/env python3
"""Validate structural-wordiness inputs, judgments and full-page measurements."""
import argparse
...
PARENT = ROOT.parent/'2026-09-18-discourse-patterns'
spec = importlib.util.spec_from_file_location('discourse_evidence', PARENT/'tools/summarize.py')
previous = importlib.util.module_from_spec(spec)
spec.loader.exec_module(previous)
audit, prior = previous.audit, previous.prior
metrics = previous.metrics
# Construction matches can support wordiness edits, not claims about authorship
# or the truth of the surrounding technical behavior.
metrics.SEMANTICS.update({
    'filler.nominal-support': {'wordiness'},
    'repetition.redundant-support': {'wordiness', 'needless_repetition'},
})
...
def data_sets():
    ...
    sets['exposed_discourse'] = sets.pop('confirmation')
...
def inherited_claims(split, profile):
    old_split = 'confirmation' if split == 'exposed_discourse' else split
    # The previous candidate was rejected. Inherit its baseline judgments,
    # then explicitly review the merged quotation repair and this candidate.
    return (audit.read(PARENT/'reports'/old_split/'before'/(profile+'.json.gz')),
            audit.read(PARENT/'dispositions.json')[old_split][profile]['before'])
    ...
    for split, (pages, files, events) in sets.items():
        ...
        for profile in ('technical', 'strict'):
            ...
            if split == 'confirmation':
                summary['sensitivity'] = dict(method='No population interval: expanded convenience frame and no fresh long Ptah pages',
                                              reason='Several repository/length cells have one page; preserve observed page counts')
            if split == 'confirmation':
