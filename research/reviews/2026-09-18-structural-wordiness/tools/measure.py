...
def main():
    ...
    parser.add_argument('--set', choices=['development', 'confirmation', 'exposed_repetition', 'exposed_framing', 'exposed_local', 'exposed_context', 'exposed_instruction', 'exposed_construction', 'exposed_purpose', 'exposed_verb', 'exposed_relations', 'exposed_projection', 'exposed_scope', 'exposed_proposition', 'exposed_clauses', 'exposed_action', 'exposed_roles', 'exposed_boundaries', 'exposed_grammar', 'exposed_discourse'], required=True)
    args = parser.parse_args()
    binary = args.binary.resolve(strict=True)
    output = args.output.resolve()
    require(not output.exists(), 'Output must be new')
    for name, digest in read(ROOT/'annotation-freeze.json')['files'].items():
        require(sha((ROOT/name).read_bytes()) == digest, 'Annotation/input drift')
    sets = {'exposed_discourse': ROOT.parent/'2026-09-18-discourse-patterns/confirmation', 'exposed_grammar': ROOT.parent/'2026-09-18-clause-grammar/confirmation', 'exposed_boundaries': ROOT.parent/'2026-09-17-clause-boundaries/confirmation', 'exposed_roles': ROOT.parent/'2026-09-17-discourse-roles/confirmation', 'exposed_action': ROOT.parent/'2026-09-17-action-grammar/confirmation', 'exposed_clauses': ROOT.parent/'2026-09-17-instruction-clauses/confirmation', 'exposed_proposition': ROOT.parent/'2026-09-17-proposition-repetition/confirmation', 'exposed_scope': ROOT.parent/'2026-09-17-rhetoric-scope/confirmation', 'exposed_projection': ROOT.parent/'2026-09-17-instruction-projection/confirmation', 'development': DEVELOPMENT, 'confirmation': ROOT/'confirmation',
    ...
    # File discovery honors an enclosing Git index. Isolate restored sources
    # so untracked replay files are not silently omitted inside a checkout.
    with tempfile.TemporaryDirectory(prefix='unswell-structural-') as temporary:
