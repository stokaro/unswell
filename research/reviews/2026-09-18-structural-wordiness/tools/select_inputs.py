...
SEED = 'structural-wordiness-confirmation-v1'
...
def main():
    ...
    parser.add_argument('--expanded-sources', type=Path, required=True)
    ...
    previous = [original] + [review/('2026-09-17-'+name)/'confirmation/manifest.json'
                            for name in ('framing-recall', 'repetition-scope', 'local-repetition', 'context-recall', 'instruction-recall', 'construction-recall', 'purpose-recall', 'verb-scaffolding', 'rhetoric-relations', 'instruction-projection', 'rhetoric-scope', 'proposition-repetition', 'instruction-clauses', 'action-grammar', 'discourse-roles', 'clause-boundaries')]
    previous.append(review/'2026-09-18-clause-grammar/confirmation/manifest.json')
    previous.append(review/'2026-09-18-discourse-patterns/confirmation/manifest.json')
    ...
    expanded = {}
    licenses = {'grpc/grpc': 'Apache-2.0', 'golang/proposal': 'BSD-3-Clause',
                'rust-lang/book': 'MIT OR Apache-2.0', 'kubernetes/community': 'Apache-2.0'}
    for record_path in sorted(args.expanded_sources.glob('*/selection.json')):
        record = read(record_path)
        repo = record['repository']
        expanded[repo] = dict(path=record_path.parent.name+'/selection.json', sha256=digest(record_path.read_bytes()))
        for entry in record['selected']:
            frame.append(dict(cohort='historical', repository=repo, original_path=entry['path'],
                input_path=repo.replace('/','__')+'/'+entry['path'], words=entry['words'],
                paragraphs=entry['paragraphs'], sha256=entry['sha256'], bytes=entry['bytes'],
                format='markdown', snapshot=record['snapshot'], reference=entry['reference'],
                origin=dict(label='unknown', scope='repository', evidence='Dated snapshot; no unit-level authorship record.', generation_record=''),
                rights=dict(license=licenses[repo], evidence='Retained root notices at '+record['snapshot'], allowed_uses=['annotation','evaluation','training']),
                notices=record['notices'], expanded_pool=True))
    for page in frame:
    ...
    for cohort in ('ptah','historical'):
        order = ('long','medium','short') if cohort == 'historical' else ('short','medium')
        for length in order:
            pool = [p for p in frame if p['cohort'] == cohort and p['length_stratum'] == length
                    ...
                    and (cohort != 'historical' or p['format'] == 'markdown')]
            eligible = sorted((p for p in pool if cohort != 'historical' or p['repository'] not in repos), key=rank)
            wanted = ({'short': 4, 'medium': 2} if cohort == 'ptah' else {'short': 2, 'medium': 2, 'long': 2})[length]
            chosen = []
            ...
            cells.append(dict(cohort=cohort,length=length,available=len(pool),eligible=len(eligible),
                              requested=wanted,selected=len(chosen)))
            if len(chosen) != wanted:
                raise ValueError('Source cell shortage: '+str(cells[-1]))
            selected.extend(chosen)
    if len({p['repository'] for p in selected if p.get('expanded_pool')}) < 2:
        raise ValueError('Require at least two new repositories')
    files, pages = {}, []
    for page in selected:
        ...
        if page.get('expanded_pool'):
            root = args.expanded_sources
    ...
    result = dict(version=1,selection=SEED,expanded_pool_records=expanded,excluded_reviewed_references=sorted(references),
