...
class EvidenceTests(unittest.TestCase):
    ...
    def test_cross_page_credit(self):
        ...
        rows[0].update(status='actionable', events=['c03-d10'])
    ...
    def test_incidental_length_overlap(self):
        rows = copy.deepcopy(self.rows)
        rows[2].update(status='actionable', events=['c02-d09'])
        with self.assertRaisesRegex(ValueError, 'Unsupported credit'):
            self.check(self.report, rows)
    ...
    def test_partial_is_not_full_recall(self):
        pages, _, events = m.data_sets()['exposed_projection']
        report = m.audit.read(m.ROOT/'reports/exposed_projection/after/strict.json.gz')
        rows = m.audit.read(m.ROOT/'dispositions.json')['exposed_projection']['strict']['after']
        _, found = m.metrics.page_metrics(report, rows, pages, events, True)
        self.assertTrue(any('c01-d03' in r.get('partial_events', []) for r in rows))
        self.assertNotIn('c01-d03', found)

    def test_complete_announcement_changes_partial_to_full(self):
        pages, _, events = m.data_sets()['exposed_projection']
        evidence = m.audit.read(m.ROOT/'dispositions.json')['exposed_projection']['strict']
        for phase, expected in [('before', False), ('after', True)]:
            report = m.audit.read(m.ROOT/'reports/exposed_projection'/phase/'strict.json.gz')
            _, found = m.metrics.page_metrics(report, evidence[phase], pages, events, True)
            self.assertEqual('c02-d02' in found, expected)
