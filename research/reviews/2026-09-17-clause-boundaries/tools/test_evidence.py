...
class EvidenceTests(unittest.TestCase):
    ...
    def test_confirmation_losses_remain_visible(self):
        data = m.audit.read(m.ROOT/'dispositions.json')['confirmation']['strict']
        for phase, expected in [('before', {'c04-d08','c04-d10','c04-d13','c04-d14'}), ('after', {'c04-d08','c04-d13'})]:
    ...
    def test_named_function_detection_restored(self):
    ...
    def test_false_positive_repairs_are_not_recall_gains(self):
        data = m.audit.read(m.ROOT/'dispositions.json')
        for name in ['exposed_roles','exposed_local']:
            self.assertEqual(len(data[name]['strict']['removed']), 1)
            row = data[name]['strict']['removed'][0]
            self.assertEqual(row['status'], 'nonactionable')
            self.assertEqual(row['events'], [])

    def test_additional_repairs_cannot_increase_recall(self):
        rows = copy.deepcopy(self.rows)
        rows[33]['events'] = ['c04-d01']
    ...
    def test_incidental_length_does_not_credit_wordiness(self):
        rows = copy.deepcopy(self.rows)
        rows[1].update(status='actionable', events=['c02-d02'])
        with self.assertRaisesRegex(ValueError, 'Unsupported credit'):
            self.check(rows)
