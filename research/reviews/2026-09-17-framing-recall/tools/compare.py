...
def evaluate(root=ROOT):
    ...
    previous = audit.read(DEVELOPMENT/'dispositions.json')
    ...
    for split,pages,files,events in [('development',dp,df,de),('confirmation',cp,cf,ce)]:
        ...
        for profile in ('technical','strict'):
            ...
            if split == 'development':
                audit.require(before == audit.read(DEVELOPMENT/(profile+'.json.gz')), 'Development baseline differs')
                bclaims = previous[profile]
