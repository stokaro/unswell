#!/usr/bin/env python3
"""Validate proposition-repetition inputs, judgments and full-page measurements."""
import argparse
...
PARENT = ROOT.parent/'2026-09-17-rhetoric-scope'
spec = importlib.util.spec_from_file_location('rhetoric_scope_evidence', PARENT/'tools/summarize.py')
...
def data_sets():
    ...
    sets['exposed_scope'] = sets.pop('confirmation')
...
def inherited_claims(split, profile):
    old_split = 'confirmation' if split == 'exposed_scope' else split
...
def validate_abstentions(report, split):
    version = '2' if report['manifest']['tool_commit'] == audit.read(ROOT/'engines.json')['after'] else '1'
    expected = [dict(path='sources/c05.md', rule_id='repetition.repeated-claim', rule_version=version,
...
def run(split, phase, profile, pages, files):
    # Exposed instruction page c05 retains one existing budget abstention.
    # Preserve it without weakening source, policy or completeness checks.
...
def validate_prototype(prototype, final, split):
    audit.require(prototype['manifest']['tool_commit'] == 'b963a42da231240e718b3e3f965ed9f51e546f5b',
                  'Prototype revision drift')
    audit.require(prototype['findings'] == final['findings'] and prototype['documents'] == final['documents']
                  and prototype['gate'] == final['gate'], 'Prototype semantic drift')
    output_fields = ('tool_commit', 'abstained_rules')
    audit.require({k: v for k, v in prototype['manifest'].items() if k not in output_fields} ==
                  {k: v for k, v in final['manifest'].items() if k not in output_fields}, 'Prototype policy drift')
    audit.require(prototype['status'] == 'complete' and prototype['manifest']['complete']
                  and not prototype['errors'], 'Incomplete prototype')
    expected = final.get('abstentions', [])[:]
    extra = {'exposed_framing': 'sources/c06.mdx', 'exposed_scope': 'sources/c03.md'}
    if split in extra:
        expected.append(dict(path=extra[split], rule_id='repetition.repeated-claim', rule_version='2',
                             reason='budget_exhausted', detail='repetition work exceeds max_candidates'))
    audit.require(prototype.get('abstentions', []) == expected, 'Prototype abstention drift')
    audit.require(prototype['manifest'].get('abstained_rules', []) ==
                  sorted({a['rule_id'] for a in expected}), 'Prototype abstention manifest drift')
...
def evaluate():
    ...
    for split, (pages, files, events) in sets.items():
        ...
        for profile in ('technical', 'strict'):
            ...
            prototype_dir = ROOT/'prototype-reports'/split/'after'
            prototype_path = prototype_dir/(profile+'.json.gz')
            prototype_record = audit.read(prototype_dir/'runs.json')[profile]
            audit.require(audit.sha(prototype_path.read_bytes()) == prototype_record['sha256'], 'Prototype report drift')
            validate_prototype(audit.read(prototype_path), after, split)
