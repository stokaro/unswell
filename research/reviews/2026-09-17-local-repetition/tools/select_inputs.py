"""Freeze twelve confirmation pages, excluding every earlier complete-page review."""
import argparse
import gzip
import hashlib
import io
...
def main():
    ...
    used = {p['reference'] for p in manifest['pages'] + json.loads(previous.read_text())['pages']
            + json.loads(scope.read_text())['pages']}
    pages = []
    for cohort in ('ptah', 'historical'):
        for fmt in ('any',):
            for length in ('short', 'medium', 'long'):
                ...
                def rank(p):
                    return digest(('local-repetition-confirmation-v1\n'+p['reference']).encode())
                if len(candidates) < 2:
                    raise ValueError('Insufficient unexposed source identities')
    ...
    result = dict(version=1, selection='local-repetition-confirmation-v1', development_manifest_sha256=digest(development.read_bytes()),
