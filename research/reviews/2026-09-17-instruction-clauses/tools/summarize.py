#!/usr/bin/env python3
"""Validate instruction-clause inputs, judgments and full-page measurements."""
import argparse
...
PARENT = ROOT.parent/'2026-09-17-proposition-repetition'
spec = importlib.util.spec_from_file_location('proposition_evidence', PARENT/'tools/summarize.py')
...
def data_sets():
    ...
    sets['exposed_proposition'] = sets.pop('confirmation')
...
def inherited_claims(split, profile):
    old_split = 'confirmation' if split == 'exposed_proposition' else split
...
def validate_abstentions(report, split):
    expected = [dict(path='sources/c05.md', rule_id='repetition.repeated-claim', rule_version='2',
