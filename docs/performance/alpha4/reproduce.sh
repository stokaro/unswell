...
asset=unswell_0.1.0-alpha.4_linux_amd64.tar.gz
curl -fsSL --retry 3 "https://github.com/stokaro/unswell/releases/download/v0.1.0-alpha.4/$asset" -o "$temporary/$asset"
actual=$(sha "$temporary/$asset")
test "$actual" = 4bf7e30f45c5f8144a5c3f1ba6d23e785fd1c0687ebfe5bf33311d5360f106b2
tar -xzf "$temporary/$asset" -C "$temporary"
mkdir -p "$temporary/payload/scripts" "$temporary/payload/sources"
cp "$temporary/unswell_0.1.0-alpha.4_linux_amd64/unswell" "$temporary/payload/unswell"
actual=$(sha "$temporary/payload/unswell")
test "$actual" = ea614783f559c44c272c470e7cba891c095850130a23ccd9a35cebb2f91be50c
cp scripts/measure-performance.sh scripts/summarize-performance.py "$temporary/payload/scripts/"
sed "s/diabolocom-alpha4-/$context-alpha4-/" docs/performance/alpha4/run.sh >"$temporary/payload/run.sh"
jq -r '.[] | [.name, .repository, .commit, .archive_sha256] | @tsv' docs/performance/alpha4/inputs.json >"$temporary/inputs.tsv"
...
docker --context "$context" build --tag "$image" docs/performance/alpha4
built=true
...
cp "$root/docs/performance/alpha4/inputs.json" "$output/inputs.json"
