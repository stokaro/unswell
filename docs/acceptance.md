...
| D. Confirmatory study | Not met | The [confirmatory measurement](../research/methods/llm-patterns-v2.md) ran on the `final_test` partition on September 14, 2026. Neither frozen hypothesis is supported: `syntax.noun-stack` gives +0.41 points against a development estimate of +2.96, `readability.grade-metric` +0.32 against +2.43, both intervals contain zero, both p-values exceed 0.72, and Holm rejects neither. The controlled arm also holds 16 provenance components against the cluster minimum of 20, so both cards read `inconclusive`. Screening SHA-256: `dc56db3b3ae5d63c1a6d1bea67c885d3790f1272224af2b711cf535b0f70e0ce`. The frozen list is untouched; whether this spent the one permitted measurement is recorded as a maintainer's call. The measurement predates the extraction fix of [#279](https://github.com/stokaro/unswell/issues/279); the corpus was counted again afterwards and the protocol carries the before-and-after |
| E. Evidence release | Met | Published in [#287](https://github.com/stokaro/unswell/pull/287). The [data card](../research/methods/data-card-v1.md) describes 28,740 documents and 5,186,062 prose words over six cohorts, 76 pinned snapshots and 73 independence groups, with the licensing position and the questions the corpus cannot answer. The [report](../research/report-v1.md) states the design, the results, the negative results, the sensitivity and holdout analyses, and the limitations. `python3 scripts/reproduce-tables.py` rebuilds the tables; the evidence cards and the confirmatory result rebuild from the committed screening records alone. The release publishes stage D's null rather than working around it |
