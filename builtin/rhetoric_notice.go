...
func scopedInformationNotice(c frameClause, i int) bool {
	c.start += i
	return !rhetoricQuoted(c) && !rhetoricAttributed(c) && (informationNotice(c.tokens()) ||
		cognitiveAnnouncement(c.tokens()) || impersonalAttention(c.tokens(), c.start == 0) ||
		authorNotice(c.tokens()) || documentEndorsement(c.tokens()))
}
