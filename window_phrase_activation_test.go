...
func TestWindowPhraseActivationsRespectCandidateBoundaries(t *testing.T) {
	for _, test := range []struct {
		...
		{"inline code outside candidate", "Alpha beta with `code`.", "", "", "", []string{""}, []float64{0}, 0},
		...
		{"inline code retains cluster", "Alpha beta.\n\nUse `code`.\n\nAlpha beta.", "", "", "",
			[]string{"", "inapplicable/no_eligible_window", ""}, []float64{0.333, 0.333}, 1},
